...
class Model():
    def __init__(self, args, infer=False):
        ...
        self.chunk_pointer = tf.Variable(0, name="chunk_pointer", trainable=False, dtype=tf.int32)
        self.batch_pointer = tf.Variable(0, name="batch_pointer", trainable=False, dtype=tf.int32)
        self.inc_chunk_pointer_op = tf.assign(self.chunk_pointer, self.chunk_pointer + 1)
        ...
        def loop(prev, _):
            prev = tf.matmul(prev, softmax_w) + softmax_b #matmul矩阵相乘
            prev_symbol = tf.stop_gradient(tf.argmax(prev, 1))
            return tf.nn.embedding_lookup(embedding, prev_symbol)#前一步的输出token在字典中的向量作为下一步的输入
        ...
        #把[batch_size,seq_length,rnn_size]的向量转为[batch_size*seq_length,rnn_size]
        output = tf.reshape(tf.concat(outputs, 1), [-1, args.rnn_size]) #tf.concat(concat_dim, values)连接concat_dim维度上的俩向量
        self.logits = tf.matmul(output, softmax_w) + softmax_b
        self.probs = tf.nn.softmax(self.logits)
        #tf.nn.seq2seq.sequence_loss_by_example(logits, targets, weights) loss=-1/N((ln p*targeti)i从1到N求和)
        loss = legacy_seq2seq.sequence_loss_by_example([self.logits],
                [tf.reshape(self.targets, [-1])],#[batch_size,seq_length]的target转为一维向量[batch_size*seq_length]
                [tf.ones([args.batch_size * args.seq_length])],#创建一个所有的参数为1的tensor对象作为weight对运算结果进行加权
                args.vocab_size)
        ...
        #print("is training:%d"%self.is_training)
